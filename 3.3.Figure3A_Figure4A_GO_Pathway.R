setwd("/Users/sohyunmoon/Desktop/2023_NJ_RNAseq_Protein")
...
make_graph <- function(csv_filename, svg_filename, plot_title, plot_cb = NULL) {
  ...
  GO_top30 <- head(GO[order(GO$adjusted_p_value),], n=30)
  GO_top30$term_name <- factor(
    GO_top30$term_name,
    levels=GO_top30$term_name[rev(order(GO_top30$adjusted_p_value))]
  )

  # Add a little more on the right-side so we see the label for the
  # max p value.
  x_scooch <- scooch_out(max(GO_top30$adjusted_p_value))

  size_breaks <- scales::breaks_extended()(GO_top30$intersection_size)
  ...
  size_breaks[1] <- floor(min(GO_top30$intersection_size))
  size_breaks[length(size_breaks)] <- ceiling(max(GO_top30$intersection_size))

   
CairoSVG(svg_filename, width = 8, height = 10, unit = "px")

    ggplot(GO_top30, aes(x = adjusted_p_value, y = term_name,
...
make_graph(csv_filename="NJ_pathway_Female_Seleted_list.csv", svg_filename="NJ_Pathway_Female_Selected_Label.svg",plot_titl="GO Female-enrichment genes") 
make_graph(csv_filename="NJ_pathway_Male_Seleted_list.csv", svg_filename="NJ_Pathway_Male_Selected_Label.svg", plot_title="GO Male-enrichment genes")

quit()


#### GO dot plot W/O name


make_graph <- function(csv_filename, svg_filename, svg_filename2, plot_title, plot_cb = NULL) {
  print(csv_filename)
...
make_graph(csv_filename="NJ_pathway_Female_Seleted_list.csv", svg_filename="NJ_Pathway_Female_Selected.svg", plot_titl="GO Female-enrichment genes") 
make_graph(csv_filename="NJ_pathway_Male_Seleted_list.csv", svg_filename="NJ_Pathway_Male_Selected.svg", plot_title="GO Male-enrichment genes")
